...
%%
%% It is highly recommended to return a timeout value from the init/2 callback to ensure that the process is terminated when no data has been received during that timespan.
%% The default timeout is infinity, which should only be used if you have alternate means of ending inactive connections.

%% http://ninenines.eu/docs/en/cowboy/HEAD/manual/cowboy_loop/
-module(cowboy_loop).
