...
%% Whenever Req is returned, you must use this returned value
%% and ignore any previous you may have had.
%% This value contains various values which are
%% necessary for Cowboy to keep track of the request and response states.
%% All functions which perform an action should only be called once.
%% This includes reading the request body or replying.
%% Cowboy will throw an error on the second call when it detects suspicious behavior.
%% It is highly discouraged to pass the Req object to another process.
%% Doing so and calling cowboy_req functions from it leads to undefined behavior.
-module(cowboy_req).
